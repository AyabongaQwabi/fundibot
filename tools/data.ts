...
const SEED_DIR = path.join(process.cwd(), 'lib/seed/final/claude/merged');
...
function loadProfileFromRich(richFile: string): { logo: string | null; short_name: string | null } {
  try {
    const fullPath = path.join(process.cwd(), 'lib/seed', richFile);
...
function loadToolsData(): ToolsData {
  ...
  const index = readJson<{ institutions: Institution[] }>(
    path.join(SEED_DIR, 'index.json'),
  );
  const programmesIndex = readJson<{ programmes: Programme[] }>(
    path.join(SEED_DIR, 'index.programmes.json'),
  );
